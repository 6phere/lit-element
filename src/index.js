...
class SixphereLitElement extends LitElement {
    ...
    bindChildren () {
        ...
        [...nodes]
            .filter((node) => node.attributes["ref"] && node.attributes["ref"].value)
            .forEach((node) => this.__bind(node.attributes["ref"].value, node))
    }
    ...
    __bind(ref, node) {
        ...
        if (this.$refs[ref.toCamelCase()] === undefined)
            this.$refs[ref.toCamelCase()] = node;
    }
}
